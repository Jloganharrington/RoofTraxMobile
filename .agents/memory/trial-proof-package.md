...
- **Frontend fetch convention in rooftrax-web**: API is at root `/api/...` (matches `customFetch`). `${import.meta.env.BASE_URL}/api/...` is WRONG — it resolves to `/rooftrax-web/api/...` which vite serves as index.html. (Signup.tsx had this bug too; fixed.)
