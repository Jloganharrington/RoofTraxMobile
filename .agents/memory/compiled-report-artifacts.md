...
**Rule 6:** GCS signed *download* URLs must be minted via the Replit sidecar (`signObjectURL` with method GET), never `file.getSignedUrl()` — the external-account credentials have no private key, so client-side signing always throws. Combined with best-effort `catch → null` semantics this failed silently: every report photo rendered "Photo unavailable" while compile/preview returned 200. Any new signed-read path must go through `getSignedDownloadUrl` (sidecar-backed), and beware `try/catch → null` hiding a 100% failure rate.
...
- The summary baseline prompt embeds the contractor-lane rules itself (Section 1); composeAiSystemPrompt must NOT also append CONTRACTOR_LANE_POLICY (duplication). Gemini compile still uses the shared module; the server-side lint is the drift-proof enforcement layer. The JSON output envelope is platform-owned (OUTPUT_FORMAT_BLOCK appended after the baseline, before company additions) — never part of the owner-supplied baseline.
