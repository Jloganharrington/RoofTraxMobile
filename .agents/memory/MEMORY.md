...
- [COC extraction and PDF](coc-extraction-and-pdf.md) — pdfkit must be externalized in build.mjs; extraction prompt must exclude subtotal rows; sign endpoint pattern.
- [Remediation Step 3 gates](remediation-step3-gates.md) — 3-C profitability gate (canViewProfitability), 3-F pipelineStage removed, 3-H contractAmount manager-only, 3-I ≤0 rejected; PD-1 done (admin stats→requireAdmin); PD-2/3 still open.
- [Team roster route namespace](team-roster-route-namespace.md) — team-roster routes live at /team/users[/:userId] (requireManagerOrAdmin); /admin/stats is requireAdmin only; task agents previously set DELETE to requireAdmin — correct is requireManagerOrAdmin + actorOutranks.
- [Orval useQuery enabled pattern](orval-usequery-enabled-pattern.md) — TanStack v5 UseQueryOptions requires queryKey; `{ query: { enabled } }` alone causes TS error; always include `queryKey: getGet…QueryKey(id)` in the override object.
...
- [PP upload-path workflow](pp-upload-path.md) — Stage 3→5 flow: /pp/new → /pp/new/intake → /pp/new/:id/estimate → /pp/wizard/:id; estimate lines stored in propertyProfile.ppEstimateLines; PP price book at GET /api/pp/price-book.
...
- [Claim supplements migration](pipeline-schema-migrations.md) — 018b_claim_supplements.sql adds supplement_id to claim_sections + report_attestations + claim_supplements table; must be applied before section generate/approve routes work.
...
- [Contract Record system](contract-record-system.md) — migration 036; OpenAPI-first; portalFetch mutator strips /api; rep-assisted select; void clears pin write-back in tx; inspection estimate prefill; sha256 binding on sign; 410 for voided; post-sign SMTP emails; all 3 tsc clean.
- [expo-notifications v57 SDK 54 quirks](expo-notifications-v57-sdk54.md) — PermissionResponse type from 'expo' drops granted/canAskAgain/status in TS; use local cast helper. shouldShowBanner+shouldShowList replaces shouldShowAlert. Push needs EAS projectId + dev build (not Expo Go).
...
- [Tenant preset-role permission policies](tenant-role-permission-policies.md) — company role policies override the shared registry but remain below explicit per-user overrides; super admins alone can edit them.
