...
- [Facet routing two-stage flow](facet-routing-two-stage.md) — EXTRACT only in analyze (one PDF call per report); SEQUENCE text-only; re-analysis clears entry+sequence; slope F-labels mirror sequence order.
